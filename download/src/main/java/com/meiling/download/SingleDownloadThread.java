...
public class SingleDownloadThread extends Thread {
    ...
    public void download() {
        ...
        try {
            ...
            String tempInfo = UpdateUtil.getFileInfoValue(mContext,FileNameHash.getSHA1String(url_string+sumSize+"_"+threadNumber));// 在保存和获取断点数据时，必须根据子线程来，所以Key中必须带有子线程编号
...
//                    currentPosition=start;// 这里不要进行这个操作，否则当重新启动时，上一次的进度会因为这个操作而被覆盖掉（加上这句相当于重新下载）
                    //  对于需要定制的话，可以考虑在这里加上一个boolean标志，通过判断这个标志来确定是否需要进行重新下载
                }
            }else{
            ...
            if (code == HttpURLConnection.HTTP_OK || code == HttpURLConnection.HTTP_PARTIAL) {
                ...
                while ((len = inputStream.read(cache)) != -1) {
                    //  保存断点规则：Key = sha1(url+文件总大小)
                    //  保存断点规则：Value = 子线程总数;当前线程编号;文件总大小;起点位置;终点位置;当前位置
                    currentPosition += len;
                    tempSum += len;
